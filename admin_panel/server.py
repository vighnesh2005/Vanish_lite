...
def usb_polkit_rule_path(username: str) -> Path:
    return Path(f"/etc/polkit-1/rules.d/49-vanish-usb-{username}.rules")
...
def stop_single_session(username: str) -> tuple[bool, str]:
    ...
    usb_polkit_rule_path(username).unlink(missing_ok=True)
